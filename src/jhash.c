...
static inline unsigned char GetCRC8(unsigned char crc, unsigned char ch)
{
	int i;
...
int crc_8(unsigned char *source, size_t number, unsigned char *crc)
{
	size_t i;
...
uchar pearson(unsigned char *key, size_t len)
{
	uchar hash;
	ub4 i;
	for (hash = len, i = 0; i < len; ++i)
...
uint32_t joat_hash(unsigned char *key, size_t key_len)
{
	uint32_t hash = 0;
	size_t i;
...
uint32_t _jac_crc(unsigned char *buf, size_t len)
{
	return joat_hash(buf, len);
}

uint16_t _jn_crc(unsigned char *buf, size_t len)
{
	return joat_hash(buf, len);
}

uint32_t _jdb_crc(unsigned char *buf, size_t len)
{
	return joat_hash(buf, len);
}
