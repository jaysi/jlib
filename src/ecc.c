...
void field_add(elem_t z, const elem_t x, const elem_t y)
{				/* field addition */
	int i;
...
void field_invert(elem_t z, const elem_t x)
{				/* field inversion */
	elem_t u, v, g, h;
	int i;
	bitstr_copy(u, x);
...
void point_double(elem_t x, elem_t y)
...
void get_random_exponent(exp_t exp)
{
	...
	do {
		...
		for (r = bitstr_sizeinbits(base_order) - 1; r < NUMWORDS * 32;
		     r++)
			bitstr_clrbit(exp, r);
...
void ECIES_generate_key_pair(void)
{				/* generate a public/private key pair */
	char buf[8 * NUMWORDS + 1], *bufptr =
	    buf + NUMWORDS * 8 - (DEGREE + 3) / 4;
	elem_t x, y;
	exp_t k;
...
ecc_keys *ECIES_generate_keys(void)
{
	char buf[8 * NUMWORDS + 1], *bufptr =
	    buf + NUMWORDS * 8 - (DEGREE + 3) / 4;
	elem_t x, y;
	exp_t k;
	ecc_keys *key = (ecc_keys *) malloc(sizeof(ecc_keys));
	int i;
	...
	point_mult(x, y, k);
//  printf("Here is your new public/private key pair:\n");
	bitstr_to_hex(buf, x);	//printf("Public key (Px:Py): %s:", bufptr); 
	for (i = 0; i < 41; i++)
		key->px[i] = bufptr[i];
	key->px[41] = '\0';
	bitstr_to_hex(buf, y);	//printf("%s\n", bufptr);
	for (i = 0; i < 41; i++)
		key->py[i] = bufptr[i];
	key->py[41] = '\0';
	bitstr_to_hex(buf, k);	//printf("Private key: %s\n", bufptr);
	for (i = 0; i < 41; i++)
		key->priv[i] = bufptr[i];
	key->priv[41] = '\0';
...
{
	return (bitstr_sizeinbits(Px) > DEGREE)
	    || (bitstr_sizeinbits(Py) > DEGREE) || point_is_zero(Px, Py)
				...
				const char *public_y, const char *private)
{
	...
	if ((ret = ECIES_decryption(decrypted, encrypted, len, private)) < 0)	/* decryption */
...
/*                                                
  ...
  ECIES_generate_key_pair();          generate a public/private key pair */
	...
	encryption_decryption_demo
	    ("This secret demo message will be ECIES encrypted", key->px,
	     key->py, key->priv);

	strcpy(k.px, "1c56d302cf642a8e1ba4b48cc4fbe2845ee32dce7");
	strcpy(k.py, "45f46eb303edf2e62f74bd68368d979e265ee3c03");
	strcpy(k.priv, "0e10e787036941e6c78daf8a0e8e1dbfac68e26d2");

	encryption_decryption_demo
	    ("This secret demo message will be ECIES encrypted", k.px, k.py,
	     k.priv);
	return 0;
}
